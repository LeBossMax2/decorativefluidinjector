/*
 * This is a modified version of ItemTextureQuadConverter
 */
...
public final class ItemTextureQuadBuilder
{
	
	private ItemTextureQuadBuilder() { }
	
	/**
	 * Takes a texture and converts it into BakedQuads. The conversion is done by scanning the texture horizontally and vertically and creating "strips" of the texture. Strips that are of the same size and follow each other are converted into one bigger quad. </br>
	 ...
	 * @param format
	 * @param template
	 *            The input texture to convert
	 * @param sprite
	 *            The texture whose UVs shall be used @return The generated quads.
	 */
		...
		for (int y = 0; y < h; y++)
		{
			for (int x = 0; x < w; x++)
			{
				...
				if (start >= 0 && !isVisible(pixel))
				{
					...
					// create the quad
					quads.add(genQuad(format, transform, start * wScale, y * hScale, x * wScale, endY * hScale, z, sprite, facing, color));
		...
		for (int x = 0; x < w; x++)
		{
			for (int y = 0; y < h; y++)
			{
				...
				if (start >= 0 && !isVisible(pixel))
				{
					...
					// create the quad
					quads.add(genQuad(format, transform, x * wScale, start * hScale, endX * wScale, y * hScale, z, sprite, facing, color));
	...
	private static UnpackedBakedQuad putQuad(VertexFormat format, TRSRTransformation transform, EnumFacing side, TextureAtlasSprite sprite, int color, float x1, float y1, float x2, float y2, float z, float u1, float v1, float u2, float v2)
	{
		...
		if (side == EnumFacing.SOUTH) // Just invert the rendered side
		{
